...
import queries.OrganizedEventsQuery;
...
public class OrganizedEvents extends ListActivity
{   
	...
	public void onCreate(Bundle savedInstanceState)
	{
        ...
        final Intent intent = new Intent().setClass(this, EditEvent.class);
	...
	{	
			...
			viewStudy = new Runnable()
			{
				public void run()
				{
					try {
						...
						OrganizedEventsQuery sq = new OrganizedEventsQuery();
						String query = getQueryString(); 
						sq.getInfo(query, "", OrganizedEvents.this, returnRes);
					} catch (IOException e) {
						e.printStackTrace();
		...
		if(!loaded)
			m_ProgressDialog = ProgressDialog.show(OrganizedEvents.this, "Please wait...", "Retrieving data", true);
	}
	
	protected String getQueryString()
	{
		return "SELECT event.id, user.email, user.phone, user.firstname, user.lastname, course.Course, course.Name, " +
				...
				"where user.id = " + HomePage.userID + " GROUP BY event.id ORDER BY event.event_name Asc";
	}
	...
	private class StudyAdapter extends ArrayAdapter<StudyObject>
	{
		...
		private class StudyNameFilter extends Filter
		{
			...
			protected void publishResults(CharSequence constraint, FilterResults results)
			{
		        ...
		        final Intent intent = new Intent(getContext(), EditEvent.class);
				ListView lv = getListView();
				lv.setTextFilterEnabled(true);
